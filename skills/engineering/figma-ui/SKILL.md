---
name: figma-ui
description: Use this skill when implementing UI from a Figma link, Figma screenshot, design mockup, or visual reference. It guides agents to prioritize existing project design tokens and components, then restore the Figma design with pixel-level fidelity, componentized code, flex-first layouts, and explicit sizing decisions.
---
...
Implement a Figma design as a front-end page or component with the highest practical visual fidelity while preserving the current project's design system, component conventions, and code style.

This is not a generic "look at Figma and write code" workflow. Treat it as a structured implementation process:

1. Analyze the Figma design.
2. Scan the project design system and existing components.
3. Map Figma styles to project tokens.
4. Reuse existing components before creating new ones.
5. Choose maintainable layout and sizing strategies.
6. Implement componentized UI.
7. Validate the result visually and technically.
...
6. Reasonable visual inference from screenshots only when structured data is unavailable.

Do not start with hardcoded values. Use hardcoded colors, spacing, typography, shadows, or radii only when the project has no matching token or when the Figma design intentionally requires a unique value.
...
Do not recreate an existing component unless the existing component cannot support the design requirement.

## Required Workflow

### 1. Analyze Figma before coding
...
- Width, height, min/max constraints, and alignment.
- Borders, radii, shadows, opacity, and z-index.
- Icons, images, SVGs, and media assets.
- Hover, active, focus, disabled, selected, loading, empty, expanded, and collapsed states.
- Responsive variants and breakpoints.
- Dynamic data states such as empty content, long text, missing images, loading, and errors.

If there are multiple frames, versions, or states and the target is unclear, ask the user which one to implement.

### 2. Scan the project

Before implementation, search for project conventions and reusable parts, including:
...
1. Figma structure analysis.
2. Project convention scan result.
3. Components to reuse.
4. Components to create.
5. Token mapping strategy.
6. Layout strategy.
7. Fixed-size versus adaptive-size strategy.
8. Asset handling strategy.
9. Responsive strategy.
10. Open questions or uncertain UI decisions.

If a key UI, interaction, responsive, sizing, asset, or business requirement is uncertain, ask the user before coding.
